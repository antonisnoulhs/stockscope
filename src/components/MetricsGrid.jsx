...
function MetricCard({ label, value }) {
  return (
    <div style={{
      ...
      <div style={{ fontFamily: 'var(--font-data)', fontSize: 18, fontWeight: 600, color: 'var(--text)' }}>
        {value}
      </div>
    </div>
  )
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 13, color: 'var(--text-dim)', marginBottom: 4 }}>
            {data.symbol} {data.sector ? `· ${data.sector}` : ''}
          </div>
          <div style={{ fontFamily: 'var(--font-ui)', fontWeight: 800, fontSize: 22, marginBottom: 2 }}>
            {data.name}
          </div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontFamily: 'var(--font-data)', fontWeight: 600, fontSize: 36 }}>
            ${fmt(data.price)} <span style={{ fontSize: 14, color: 'var(--text-dim)' }}>{data.currency}</span>
          </div>
          <div style={{
