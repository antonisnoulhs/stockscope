...
export default function StockChart({ ticker }) {
  ...
  return (
    <div className="fade-in" style={{
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {PERIODS.map(p => (
            <button
              ...
              style={{
                background: period === p.value ? 'var(--accent)' : 'var(--surface2)',
                color: period === p.value ? '#080b10' : 'var(--text-dim)',
                border: '1px solid var(--border)',
                borderRadius: 6,
                padding: '5px 12px',
                fontSize: 12,
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-dim)', fontFamily: 'var(--font-data)' }}>
          Loading chart…
        </div>
      )}
      ...
      {!loading && !error && history.length > 0 && (
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={history} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
            ...
            <XAxis
              dataKey="date"
              tick={{ fill: '#6b7e99', fontSize: 11, fontFamily: 'JetBrains Mono' }}
              tickLine={false}
              axisLine={false}
